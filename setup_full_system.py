...
def setup_backend():
    ...
    # Set up database
    if not run_command("python setup_database.py", "Setting up PostgreSQL database", backend_dir):
        print("⚠️ Database setup had issues")
    
    # Set up Redis
    if not run_command("python setup_redis.py", "Setting up Redis", backend_dir):
        print("⚠️ Redis setup had issues")
...
def test_system():
    ...
    # Test frontend (just check if files exist)
    frontend_build = Path("frontend/build")
    if frontend_build.exists():
        print("✅ Frontend build directory exists")
    else:
        print("⚠️ Frontend not built yet (run npm run build)")
...
def main():
    ...
    print("")
    print("4. Demo credentials:")
    print("   Email: demo@example.com")
    print("   Password: demo123")
    print("")
    print("🎯 Ready to generate your LinkedIn post!")
